...
unsigned int    TextureManager::loadTexture(const std::string filename, int textureType)
{
    unsigned int id = -1;
    ...
    if (m_textureMap.count(filename) != 0) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, heigth, 0, textureType, GL_UNSIGNED_BYTE, data);
